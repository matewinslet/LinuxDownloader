...
# Honor XDG base dirs so we write to writable locations inside the Flatpak
# sandbox (where $HOME/.config and $HOME/.cache are not accessible). Falls back
# to the usual ~/.config and ~/.cache on a normal install.
_XDG_CONFIG_HOME = os.environ.get("XDG_CONFIG_HOME") or os.path.join(HOME, ".config")
_XDG_CACHE_HOME = os.environ.get("XDG_CACHE_HOME") or os.path.join(HOME, ".cache")
CONFIG_DIR = os.path.join(_XDG_CONFIG_HOME, "ldm")
...
YT_DLP_TEMP_DIR = os.path.join(_XDG_CACHE_HOME, "ldm", "ytdlp-tmp")
# Browser cookies pushed from the companion extension over the bridge, kept in
# Netscape format for yt-dlp / requests. Used instead of reading the browser
# profile from disk directly, which the Flatpak sandbox forbids.
COOKIES_FILE = os.path.join(_XDG_CACHE_HOME, "ldm", "cookies.txt")
...
def _read_netscape_cookies(path):
    ...
    (domain, path, name) so new cookies can be merged in-place."""
    jar = {}
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(parts) != 7:
                    continue
                dom, incl, p, secure, expiry, name, value = parts
                jar[(dom, p, name)] = (dom, incl, p, secure, expiry, name, value, httponly)
...
def write_browser_cookies(cookies):
    """Merge cookies pushed from the browser extension into COOKIES_FILE
    (Netscape format). `cookies` is a list of WebExtension cookie objects
    ...
    with _COOKIES_LOCK:
        try:
            ...
            return
        jar = _read_netscape_cookies(COOKIES_FILE)
        for c in cookies:
            try:
                name = c.get("name")
                ...
                if host_only:
                    incl, dom = "FALSE", domain.lstrip(".")
                else:
                    incl, dom = "TRUE", (domain if domain.startswith(".") else "." + domain)
                path = c.get("path") or "/"
                secure = "TRUE" if c.get("secure") else "FALSE"
                exp = c.get("expirationDate")
                expiry = str(int(exp)) if exp else "0"
                value = c.get("value", "") or ""
                httponly = bool(c.get("httpOnly"))
                jar[(dom, path, name)] = (dom, incl, path, secure, expiry, name, value, httponly)
            except Exception:
                continue
        lines = ["# Netscape HTTP Cookie File",
                 "# Generated by Linux Download Manager from companion-extension cookies.",
                 ""]
        for (dom, incl, path, secure, expiry, name, value, httponly) in jar.values():
            prefix = "#HttpOnly_" if httponly else ""
            lines.append(f"{prefix}{dom}\t{incl}\t{path}\t{secure}\t{expiry}\t{name}\t{value}")
        try:
...
def cookie_opts():
    """yt-dlp cookie options: prefer the extension-provided cookies.txt; fall
    back to reading the Firefox profile directly on non-sandboxed installs."""
    try:
        if os.path.exists(COOKIES_FILE) and os.path.getsize(COOKIES_FILE) > 0:
            return {"cookiefile": COOKIES_FILE}
    except Exception:
        pass
    prof = get_firefox_profile()
    if prof and os.path.exists(prof):
        return {"cookiesfrombrowser": ("firefox", prof)}
    return {}
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        # JSON-body variant of the bridge: carries browser cookies (too large
        # for a GET query string) so authenticated downloads work without the
        # app reading the browser profile from disk (forbidden in the sandbox).
        try:
...
class DownloadThread(QThread):
    ...
    def download_with_requests(self):
        try:
            ...
            try:
                # Cookies provided by the companion extension over the bridge.
